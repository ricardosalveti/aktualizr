 ...
 public:
  SotaUptaneClient(Config &config_in, const std::shared_ptr<INvStorage> &storage_in,
      ...
      : config(config_in),
        ...
        package_manager(PackageManagerFactory::makePackageManager(config.pacman, config.bootloader, storage, http)),
  ...
  Uptane::Target getCurrent() { return package_manager->getCurrent(); }
